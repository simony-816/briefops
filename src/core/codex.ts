...
export async function installCodexPack(options: CodexInstallOptions = {}): Promise<{
  ...
  await writeTextFile(
    ...
    [
      ...
      "",
      "Start a Codex task with the BriefOps environment gate:",
      "",
      "```bash",
      "command -v briefops >/dev/null 2>&1",
      "```",
      "",
      "If `briefops` is not on `PATH`, stop and report `Status: setup-required`. Do not continue by silently skipping BriefOps.",
      "",
      "After the gate passes, load compact BriefOps context:",
      "",
      "```bash",
